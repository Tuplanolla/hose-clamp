...
long_arm = (hole_separation / 2 + hose_diameter / 2) +
           (hole_separation / 2 - boss_diameter / 2);
...
// echo("ERROR: CGAL error in CGAL_Nef_polyhedron3(): CGAL ERROR: assertion violation! Expr: e_below != SHalfedge_handle() File: /usr/include/CGAL/Nef_3/SNC_FM_decorator.h Line: 418");
...
render()
  difference() {
    minkowski() {
      ...
      difference() {
        union() {
          move([- hole_separation / 2, 0, 0])
            cyl(d = diameter + 2 * thickness - rounding,
                h = hose_length - rounding);

          /*
          rot([0, 90, 0])
            skin(profiles = [rect([h - rounding, diameter + 2 * thickness - rounding]),
                             rect([boss_diameter - rounding, bolt_length + gap_size - rounding])],
                 z = [- hole_separation / 2, hole_separation / 2],
                 slices = 0);
          */
          ...
          rot([0, 90, 0])
            ...
            skin(profiles = [for (i = [0 : $fn - 1])
                             rect(lerp([hose_length - rounding, diameter + 2 * thickness - rounding],
                                       [boss_diameter - rounding, bolt_length + gap_size - rounding],
                                       deform_range(i / ($fn - 1))))],
                 z = [for (i = [0 : $fn - 1])
                      lerp(- hole_separation / 2,
                           hole_separation / 2,
                           deform_domain(i / ($fn - 1)))],
                 slices = 0);

          move([hole_separation / 2, 0, 0])
            rot([90, 0, 0])
            cyl(d = boss_diameter - rounding,
